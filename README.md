...
- Deploy SealedSecrets controller via helm chart globally
...
- Label your local hub cluster with `ztp-gitops-sno=true` to deploy a sno bm cluster
- Label your cluster(s) with `update-cluster-version=true` to update the cluster version

VMC

- Label your local hub cluster with `ztp-assisted-service-vanilla=true` to setup provisioning
- Label your local hub cluster with `ztp-gitops-vmc-helm=true` to deploy a vmc cluster
...
```bash
kubeseal < secrets/cluster-7xbw7/01-vsphere-creds-secret.yaml > gitops-vmc-policy/input/clusters/7xbw7/01-vsphere-creds-sealed-secret.yaml \
    -n cluster-7xbw7 \
    --controller-namespace sealed-secrets \
    --controller-name sealed-secrets \
    -o yaml

kubeseal < secrets/cluster-7xbw7/01-cluster-install-secret.yaml > gitops-vmc-policy/input/clusters/7xbw7/01-cluster-install-sealed-secret.yaml \
    -n cluster-7xbw7 \
    --controller-namespace sealed-secrets \
    --controller-name sealed-secrets \
    -o yaml

kubeseal < secrets/cluster-7xbw7/01-cluster-pull-secret.yaml > gitops-vmc-policy/input/clusters/7xbw7/01-cluster-pull-sealed-secret.yaml \
    -n cluster-7xbw7 \
    --controller-namespace sealed-secrets \
    --controller-name sealed-secrets \
    -o yaml

kubeseal < secrets/cluster-7xbw7/01-cluster-ssh-private-key-secret.yaml > gitops-vmc-policy/input/clusters/7xbw7/01-cluster-ssh-private-key-sealed-secret.yaml \
    -n cluster-7xbw7 \
    --controller-namespace sealed-secrets \
    --controller-name sealed-secrets \
    -o yaml

kubeseal < secrets/cluster-7xbw7/01-cluster-vsphere-certs-secret.yaml > gitops-vmc-policy/input/clusters/7xbw7/01-cluster-vsphere-certs-sealed-secret.yaml \
    -n cluster-7xbw7 \
    --controller-namespace sealed-secrets \
    --controller-name sealed-secrets \
    -o yaml
